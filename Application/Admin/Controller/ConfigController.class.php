...
// 		    $this->saveConfigCache();
	...
	public function moble()
	{
		...
		if (!in_array('moble_type', $Config_DbFields)) {
			M()->execute('ALTER TABLE `movesay_config` ADD COLUMN `moble_type` VARCHAR(200) DEFAULT \'\' NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}

		if (!in_array('moble_key', $Config_DbFields)) {
			M()->execute('ALTER TABLE `movesay_config` ADD COLUMN `moble_key` VARCHAR(200) DEFAULT \'\' NOT NULL   COMMENT \'名称\' AFTER `id`;');
		}
	...
	public function coinEdit($id = NULL)
	{
		if (empty($_POST)) {
			...
			if ($_POST['id']) {
				...
				$rea = M()->execute('ALTER TABLE  `movesay_user_coin` ADD  `' . $_POST['name'] . '` DECIMAL(20,8) UNSIGNED DEFAULT 0.00000000 NOT NULL');
				$reb = M()->execute('ALTER TABLE  `movesay_user_coin` ADD  `' . $_POST['name'] . 'd` DECIMAL(20,8) UNSIGNED DEFAULT 0.00000000 NOT NULL ');
				$rec = M()->execute('ALTER TABLE  `movesay_user_coin` ADD  `' . $_POST['name'] . 'b` VARCHAR(200) DEFAULT \'\' NOT NULL ');
				$rs = M('Coin')->add($_POST);
			}
	...
	public function textEdit($id = NULL)
	{
		if (empty($_POST)) {
			...
			if ($_POST['id']) {
				...
				D('Text')->reload_content_by_id($_POST['id']);
			}
			else {
...
// 		    $this->saveConfigCache();
	...
	public function daohangReload()
	{
        if (APP_DEMO) {
            $this->error('测试站暂时不能修改！');
        }
        $daohang = M('Daohang')->where(array('status' => 1))->order('sort asc')->select();
        S('daohang', $daohang);
        if ($daohang) {
            $this->success('重载成功！');
        }
	...
	public function checkUpdata()
	{
		if (!S(MODULE_NAME . CONTROLLER_NAME . 'checkUpdata')) {
			...
			if (!in_array('footer_logo', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_config` ADD COLUMN `footer_logo` VARCHAR(200) DEFAULT \'\' NOT NULL   COMMENT \' \' AFTER `id`;');
			}

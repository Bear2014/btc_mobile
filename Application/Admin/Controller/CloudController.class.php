...
class CloudController extends AdminController
{
	...
	public function kefu()
	{
		...
		if (!in_array('kefu', $DbFields)) {
			M()->execute('ALTER TABLE `movesay_config` ADD COLUMN `kefu` VARCHAR(200) DEFAULT \'\'  NOT NULL   COMMENT \'\' AFTER `id`;');
		}
	...
	public function theme()
	{
		...
		if (!in_array('index_html', $DbFields)) {
			M()->execute('ALTER TABLE `movesay_config` ADD COLUMN `index_html` VARCHAR(200) DEFAULT \'\' NOT NULL   COMMENT \'\' AFTER `id`;');
		}
	...
	public function checkUpdata()
	{
		if (!S(MODULE_NAME . CONTROLLER_NAME . 'checkUpdata')) {
			...
			if (!in_array('class', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_version_game` ADD COLUMN `class` VARCHAR(200) DEFAULT \'\'  NOT NULL   COMMENT \'名称\' AFTER `id`;');
			}

			if (!in_array('shuoming', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_version_game` ADD COLUMN `shuoming` VARCHAR(200) DEFAULT \'\' NOT NULL   COMMENT \'名称\' AFTER `id`;');
			}

			if (!in_array('gongsi', $DbFields)) {
				M()->execute('ALTER TABLE `movesay_version_game` ADD COLUMN `gongsi` VARCHAR(200) DEFAULT \'\' NOT NULL   COMMENT  \'名称\' AFTER `id`;');
			}
